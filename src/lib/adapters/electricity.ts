...
export async function fetchSandbox(ctx: AdapterContext): Promise<ElectricityMetadata> {
  const base = process.env.IDBI_ELEC_BASE_URL;
  const key = process.env.IDBI_ELEC_API_KEY;
  if (!base || !key) return fetchMock(ctx);
  const res = await fetch(`${base.replace(/\/$/, "")}/consumer/${encodeURIComponent(ctx.pan)}/usage`, {
